...
# ASYNC SAVE (after call)
async def async_save(payload):
    await asyncio.sleep(2)  # wait for call to fully end
...
# ---------------------------------------------------------
# VOICE FLOW (FAST: no waiting)
# ---------------------------------------------------------
def gather(vr, action, prompt, timeout=6):
    g: Gather = vr.gather(
        input="speech",
        action=action,
        method="POST",
...
async def voice_name(request: Request):
    name = (await request.form()).get("SpeechResult", "Guest")
    vr = VoiceResponse()
    gather(vr, f"/voice/party?name={name}", f"Nice to meet you {name}. For how many people?")
    return Response(content=str(vr), media_type="application/xml")

@app.post("/voice/party")
async def voice_party(request: Request, name: str):
    speech = (await request.form()).get("SpeechResult", "")
    party = next((token for token in speech.split() if token.isdigit()), "1")

    vr = VoiceResponse()
    gather(vr, f"/voice/datetime?name={name}&party={party}",
           "What date and time should I book? Example: Friday at 7 PM.", timeout=10)
    return Response(content=str(vr), media_type="application/xml")

@app.post("/voice/datetime")
async def voice_datetime(request: Request, name: str, party: str):
    dt = (await request.form()).get("SpeechResult", "")

    if not normalize_to_utc(dt):
        vr = VoiceResponse()
        gather(vr, f"/voice/datetime?name={name}&party={party}",
               "Sorry, I did not understand. Say: Friday, November 22 at 8 PM.")
        return Response(content=str(vr), media_type="application/xml")

    vr = VoiceResponse()
    gather(vr, f"/voice/notes?name={name}&party={party}&dt={dt}",
           "Got it. Any notes or preferences? You can say none.")
    return Response(content=str(vr), media_type="application/xml")

@app.post("/voice/notes")
async def voice_notes(request: Request, name: str, party: str, dt: str):
    n = (await request.form()).get("SpeechResult", "")
    notes = "none" if n.lower() in ["", "no", "none", "nope"] else n

    payload = {"customer_name": name, "party_size": party, "datetime": dt, "notes": notes}

    vr = VoiceResponse()
    vr.say("Perfect, I’m booking your table now.")
